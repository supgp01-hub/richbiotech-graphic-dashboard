...
assert.ok(source.includes('function closeCombinedAfterSave()'));
assert.ok(source.includes('window.lvCloseModal(true)'));
...
assert.ok(source.includes('function specialRecordList(p)'));
...
assert.ok(!source.includes('<b>ว่างรับงาน:</b>'));
assert.ok(source.includes('data-combined-mode="report"'));
assert.ok(source.includes('ประวัติวันหยุดปกติ'));
assert.ok(source.includes('ประวัติวันทำงานพิเศษ'));
assert.ok(source.includes('ประวัติรายการของฉัน'));
assert.ok(source.includes('เลือกเดือนและปีเพื่อดูรายการย้อนหลังทั้งหมด'));
assert.ok(source.includes('function normalizeManualDate(value)'));
assert.ok(source.includes('function dateEntryMarkup(prefix,value)'));
...
assert.ok(css.includes('.lvw-record-filters'));
assert.ok(css.includes('.lvw-date-entry'));
...
assert.ok(source.includes("window.rbLeaveFinishSave({button:button,promise:saved,nonBlocking:true})"));
assert.ok(source.includes("window.rbLeaveFinishSave({button:button,promise:sync,nonBlocking:true})"));
