...
assert.ok(index.includes('snippets/login-reliability-v1.js?v=207'), 'login reliability fix must be loaded');
assert.ok(index.includes('snippets/login-reliability-v1.css?v=207'), 'the lightweight responsive login design must be loaded');
assert.ok(index.indexOf('login-reliability-v1.js?v=207') < index.indexOf('rb-global-dropdown-v5-css'), 'login fix must opt out before global dropdown enhancement');
...
assert.ok(css.includes('#rb-pin-dots{display:flex!important'), 'the original four PIN boxes must remain visible');
assert.ok(css.includes('pointer-events:auto!important'), 'the PIN overlay must remain directly tappable on mobile');
