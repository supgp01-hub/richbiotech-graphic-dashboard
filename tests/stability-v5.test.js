...
assert.ok(index.length < 640000, 'initial dashboard HTML should stay below 640 KB');
...
assert.ok(performance.includes('function writeProtectedRows(')&&performance.includes('merged=destructive?rows:mergeById(remote,rows)')&&performance.includes("'if-match':etag"), 'a stale device must preserve the complete online tracker in a conditional union write');
