...
assert.ok(index.includes('snippets/order-revision-experience-v1.js?v=fix'),'the live page must load the revision experience runtime');
assert.ok(index.includes('snippets/order-revision-experience-v1.css?v=fix'),'the live page must load the revision experience styles');
...
assert.ok(experience.includes('applyLightboxZoom'),'the large preview must support fit-to-screen and controlled zoom');
assert.ok(experience.includes('rb-el-meta-toggle'),'the image details must be collapsible to maximize preview space');
...
assert.ok(css.includes('width:min(1440px,98vw)'),'the evidence preview must use almost the full desktop viewport');
