...
(async()=>{
 ...
 assert.equal(w.document.querySelector('.cts-history').hidden,true,'history starts collapsed');
 assert.equal(w.document.querySelectorAll('.ctg-submission').length,0,'no duplicate view action');
 assert.equal(w.document.querySelectorAll('.ctg-employees th').length,4,'compact four-column table');
 w.document.querySelector('.cts-history-toggle').click();assert.equal(w.document.querySelector('.cts-history').hidden,false);
 await w.ctSubmissions.refresh();assert.equal(w.document.querySelector('.cts-history').hidden,false,'refresh retains expanded history');
 w.document.querySelector('.cts-latest-text').dispatchEvent(new w.KeyboardEvent('keydown',{key:'Enter',bubbles:true}));assert(w.document.querySelector('.cts-latest-text').classList.contains('is-expanded'),'full text remains accessible by keyboard');
 w.document.querySelector('.cts-history-toggle').click();assert.equal(w.document.querySelector('.cts-history').hidden,true);
 ...
 assert.equal(w.document.querySelectorAll('.cts-inline .cts-cell-summary').length,0,'editor does not repeat the current text');
