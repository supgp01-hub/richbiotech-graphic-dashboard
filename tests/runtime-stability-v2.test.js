...
assert.ok(index.includes('snippets/bulk-import-v2.js?v=214'), 'content cloud cache must be refreshed');
assert.ok(index.includes('snippets/performance-v4.js?v=fix339'), 'content sync cache must be refreshed');
...
assert.ok(cloud.includes('window.rbFirebaseAuth.ready.then'), 'hydration must run even when authentication completed before the tracker script registered its event listener');
