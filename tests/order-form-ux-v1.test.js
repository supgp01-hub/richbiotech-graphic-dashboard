...
assert(source.includes("if(_omIsNew&&state&&state.ok){clearOMDraft();_omIsNew=false;_omEditingKey=entry._fbKey;_omReservedId='';}"), 'a durable accepted order must become an edit to avoid creating a duplicate on retry');
