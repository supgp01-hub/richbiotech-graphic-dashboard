...
assert(runtime.includes('if(pendingSync)'),'cloud reads must wait while a multi-row save is still uploading');
assert(runtime.includes('callback(null,localRows())'),'a failed sync must keep the latest local rows instead of restoring stale cloud data');
assert(runtime.includes('syncDraftFieldsFromDom();'),'the Save button must capture current input values before building records');
assert(runtime.includes("dispatchEvent(new Event('change',{bubbles:true}))"),'draft controls must be synchronized even while a field remains focused');
assert(runtime.includes('reliableSave.__rbTeamAccess=!!original.__rbTeamAccess'),'role and reliability wrappers must not stack repeatedly');
...
assert(html.includes('snippets/idcard-save-reliability-v1.js?v=fix372'),'reliability runtime must be loaded');
assert(html.includes('snippets/idcard-save-reliability-v1.css?v=fix372'),'reliability styles must be loaded');
