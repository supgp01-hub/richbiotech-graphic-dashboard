...
assert.ok(indexSource.includes("list-facebook-editor.js?v=fix384"), 'the deployed page must cache-bust the current Facebook editor');
...
assert.ok(indexSource.includes("list-facebook-followup.js?v=fix386"), 'the deployed page must load the follow-up workflow');
assert.ok(indexSource.includes('<meta name="rb-build" content="fix387">'), 'the deployed page must expose its current build for cache diagnosis');
...
assert.ok(editorSource.includes('window._listfbRefreshFromCloud=refreshCentralData'), 'audit refresh must be able to fetch the latest shared Facebook account edits');
