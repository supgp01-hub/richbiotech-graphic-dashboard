...
assert.ok(index.includes('snippets/manual-refresh-v1.js?v=fix438'), 'manual refresh controller must be deployed');
assert.ok(source.includes("function canRefresh(){var role=window._rbUser&&window._rbUser.role;return role==='sup'||role==='spec'||role==='audit'}"), 'only Supervisor, Specialist, and Audit may refresh Facebook Pages and List Facebook');
...
assert.ok(index.includes("['sup','spec','audit'].indexOf(role)<0"), 'the primary Facebook refresh function must enforce the restricted roles');
