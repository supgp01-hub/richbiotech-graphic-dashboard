...
(async () => {
  const targetUrl = process.argv[2] || 'http://127.0.0.1:8014/tests/fixtures/order-planner-harness.html?v=fix297b';
  ...
  await page.waitForFunction(() => /เลือกสินค้า/.test(document.querySelector('.rbp-content-status')?.textContent || ''));
