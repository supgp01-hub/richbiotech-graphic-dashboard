...
(async()=>{
 ...
 try{
  ...
  assert.equal(await reviewer.locator('.ctg-detail [data-content-id]').count(),0,'supervisor has no duplicate view or employee edit button');
  assert.equal(await reviewer.locator('.ctg-detail .cts-history-toggle').count(),0,'no empty history control');
  assert((await readCell.boundingBox()).height<90,'a short submission must occupy a compact row');
  ...
  await reviewer.evaluate(()=>window.ctSubmissions.refresh());assert((await readCell.innerText()).includes('ข้อความแก้จากตาราง'));assert.equal(await readCell.locator('.cts-history').isVisible(),false);
  await reviewer.locator('.ctg-detail .cts-history-toggle').click();assert((await readCell.innerText()).includes('ข้อความจากหน้ารับงาน หนึ่ง'));
  await reviewer.evaluate(()=>window.ctSubmissions.refresh());assert.equal(await readCell.locator('.cts-history').isVisible(),true,'history remains expanded after online refresh');
  ...
  await readCell.scrollIntoViewIfNeeded();await reviewer.screenshot({path:path.resolve(root,'../content-compact461-history.png'),fullPage:false});
  await reviewer.locator('.ctg-detail .cts-history-toggle').click();await reviewer.screenshot({path:path.resolve(root,'../content-compact461-default.png'),fullPage:false});
