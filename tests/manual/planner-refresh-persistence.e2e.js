...
(async()=>{
  ...
  // Reload immediately, before the 650 ms field debounce can run. pagehide and
  // the immediate add save must still leave a durable child write behind.
  ...
  await page.locator('.rbp-detail-editor [data-field="name"]').fill('งานใหม่ต้องไม่เด้งกลับ แก้ไข');
  await page.waitForTimeout(900);
  assert.ok(await page.evaluate(()=>window.rbPersistence.pendingCount())>=1,'failed cloud autosave must remain queued');
