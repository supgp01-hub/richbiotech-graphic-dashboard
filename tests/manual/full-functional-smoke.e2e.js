...
(async()=>{
  const targetUrl=process.argv[2]||'http://127.0.0.1:8011/index.html?v=fix239-functional';
  ...
  assert.strictEqual(await page.locator('.ord-tab-btn').filter({hasText:'ทีมงาน'}).evaluate(el=>getComputedStyle(el).color),'rgb(114, 210, 182)','Active order tab must use the Deep Teal accent');
