...
(async()=>{
 ...
 const worker=await contextFor('graphic','DOM'),audit=await contextFor('audit','QA Audit');
 ...
 async function returnForCorrection(){await load(audit);await open(audit,'audit');await audit.locator('.rb-om-tab').filter({hasText:'ตรวจออดิต'}).click();await audit.locator('#om-fbname').fill('QA Facebook');await audit.locator('#om-camp1').fill('QA Campaign 1');await audit.locator('#om-camp2').fill('QA Campaign 2');for(let i=0;i<2;i++){const result=audit.locator('#rb-audit-version-workflow .rb-av-result-input').nth(i);if(await result.inputValue()!=='issue')await result.selectOption('issue',{force:true});await audit.locator('#rb-audit-version-workflow .rb-av-note-input').nth(i).fill('Fix HOOK '+(i+1));}await audit.locator('#om-audit-btns button').last().click();await saved(audit);assert.equal(db.qa_rounds.status,'revision');}
 await load(worker);await open(worker,'team');await worker.locator('#om-primary-btn').click();await saved(worker);
 ...
 await load(worker);await open(worker,'team');await worker.locator('#om-submitlinks-rows input').first().fill('https://example.test/corrected1');await worker.locator('#om-primary-btn').click();await saved(worker);
 ...
 await load(worker);await open(worker,'team');await worker.locator('.rb-om-tab').filter({hasText:'ส่งงาน/สรุปงาน'}).click();
 ...
 await tableRow.locator('.rb-round-history-button').click();const history=audit.locator('dialog.rb-round-history');assert.equal(await history.locator('li').count(),5);assert.match(await history.innerText(),/พนักงานส่งแก้ครั้งที่ 2/);await history.locator('header button').click();
 ...
 assert.deepEqual(errors,[]);console.log('review rounds browser: initial submission, two correction cycles, 2 HOOKs, both submit paths, duplicate guard, table/history, approval and reload passed',puts);
