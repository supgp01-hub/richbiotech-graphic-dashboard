...
for(const width of [320,390,430,760,844,900]){
 ...
 for(const label of ['ทีมงาน','สั่งงาน','รวมลิงก์','ค่าคอมมิชชั่น','ยอดหักออดิต','Facebook Pages','List Facebook','บัตรประชาชน']){await p.locator('.gsnav-btn').filter({hasText:label}).click();await fits(label+' '+width);}
}
await p.setViewportSize({width:390,height:844});await section('team');await p.locator('.gsnav-btn').filter({hasText:'รวมลิงก์'}).click();
...
await p.locator('.gsnav-btn').filter({hasText:'List Facebook'}).click();await p.locator('#lfb-add').click();await p.locator('#lfbe-name').fill('แบบร่างที่ยังไม่บันทึก');
...
await section('team');await p.locator('.gsnav-btn').filter({hasText:'สั่งงาน'}).click();await p.locator('#ord-add-btn').click();assert.ok(await p.locator('#rb-order-modal .rb-om-window').evaluate(e=>{const r=e.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth+1}));await p.getByRole('button',{name:'ปิด',exact:true}).last().click();
await p.setViewportSize({width:1280,height:900});await p.locator('.gsnav-btn').filter({hasText:'รวมลิงก์'}).click();assert.equal(await p.locator('.ct-table').evaluate(e=>getComputedStyle(e).display),'table','desktop layout restored after rotation/resize');
