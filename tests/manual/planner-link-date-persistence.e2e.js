...
(async()=>{
  ...
  await page.locator('#ord-planner-btn').waitFor();
  await page.evaluate(()=>{
  ...
  await page.evaluate(({expectedLink})=>{
    ...
    const date=document.querySelector('.rbp-detail-editor [data-field="deadline"]');
  ...
  assert.strictEqual(await editor.locator('[data-field="deadline"]').inputValue(),'2026-09-05','Deadline must survive cloud confirmation, close and refresh');
  assert.strictEqual(await editor.locator('[data-field="scheduledDate"]').inputValue(),'2026-09-05','the synchronized planner date must follow the saved Deadline');
  ...
  assert.strictEqual(cloud.deadline,'2026-09-05','the selected Deadline must be stored online, not only in the local form');
  assert.strictEqual(cloud.scheduledDate,'2026-09-05','the synchronized planner date must be stored online with the Deadline');
