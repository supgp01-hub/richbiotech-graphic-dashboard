...
(async()=>{
  ...
  assert.strictEqual(await page.locator('#rb-evidence-lightbox .rb-el-fit').isVisible(),true,'large preview must expose fit-to-screen');
  assert.strictEqual(await page.locator('#rb-evidence-lightbox .rb-el-actual').isVisible(),true,'large preview must expose 100% zoom');
  await page.locator('#rb-evidence-lightbox .rb-el-meta-toggle').click();
  assert.strictEqual(await page.locator('#rb-evidence-lightbox .rb-el-dialog').evaluate(el=>el.classList.contains('is-meta-collapsed')),true,'image details must collapse to maximize the preview');
