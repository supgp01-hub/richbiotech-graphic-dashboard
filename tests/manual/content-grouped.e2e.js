...
test('grouped HOOK matches dates per employee and hides other submissions for staff',()=>{
 ...
 w.document.body.insertAdjacentHTML('beforeend',w.ctGrouped.row(r,1));w.document.querySelector('[data-ctg-toggle]').click();let html=w.ctGrouped.row(r,1);assert.match(html,/วันที่สั่งล่าสุด/);assert.match(html,/11\/09\/2569/);assert.match(html,/12\/09\/2569/);assert.match(html,/data-content-owner="ter"/);
