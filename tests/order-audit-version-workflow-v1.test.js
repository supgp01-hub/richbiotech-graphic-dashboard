...
assert.ok(index.includes('order-audit-version-workflow-v1.css?v=fix287'),'the version audit layout must be cache-busted');
assert.ok(index.includes('order-audit-version-workflow-v1.js?v=fix287'),'the version audit runtime must be cache-busted');
...
assert.ok(workflow.includes('issue.disabled=false'),'Audit issue type must remain editable before choosing the final result');
assert.ok(workflow.includes('note.disabled=false'),'Audit notes must remain editable before choosing the final result');
assert.ok(!workflow.includes("note.disabled=state.result!=='issue'"),'pending Audit notes must never be locked');
