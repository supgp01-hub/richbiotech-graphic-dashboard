...
const index=fs.readFileSync('index.html','utf8'),js=(fs.readFileSync('snippets/deduction-reset-policy-v1.js','utf8')+'\n'+fs.readFileSync('snippets/audit-deduction-center-v1.js','utf8')),css=fs.readFileSync('snippets/audit-deduction-center-v1.css','utf8');
assert.ok(index.includes('audit-deduction-center-v1.js?v=fix'));assert.ok(index.includes('audit-deduction-center-v1.css?v=fix'));
...
assert.ok(index.includes('audit-sheet-evidence-v1.js?v=fix470')&&js.includes('window.rbAuditSheetEvidence.read(force)'),'audit import must read the real spreadsheet hyperlink metadata');
...
assert.ok(js.includes("x.id='sheet_'+hash(sheetIdentity(x))")&&js.includes("'if-match':'null_etag'"),'sheet rows must use stable ids and skip duplicates');
...
assert.ok(js.includes('refreshSheetEvidence')&&fs.readFileSync('snippets/audit-sheet-evidence-v1.js','utf8').includes('/export?format=xlsx'),'Google Sheets import must use the cross-origin workbook export, preserving embedded URLs');
