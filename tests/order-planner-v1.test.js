...
assert.ok(/<meta name="rb-build" content="fix\d+">/.test(index),'build marker must expose the current release');
...
assert.ok(index.includes('snippets/order-planner-v1.css?v=fix'),'planner stylesheet must be loaded');
assert.ok(index.includes('snippets/order-planner-v1.js?v=fix'),'planner script must be loaded');
...
assert.ok(js.includes('setInterval(runBackgroundWorker,60000)'),'the dashboard must continuously check scheduled work');
...
const syncedDraft={status:'draft',deadline:'2026-09-12',scheduledDate:'2026-09-05'};
filter.syncPlannerDate(syncedDraft);
assert.strictEqual(syncedDraft.scheduledDate,'2026-09-12','draft schedule date must mirror Deadline');
assert.strictEqual(filter.normalizeBriefImages(['data:image/png;base64,AA==']).length,1,'legacy string images must remain visible');
const orderWithDelivery=filter.buildOrder({id:'delivery-draft',name:'งานพร้อมลิงก์',type:'กราฟิก',deliveryLink:'https://drive.google.com/delivery',briefImages:[]},'BALL',Date.now());
assert.strictEqual(orderWithDelivery.deliveryLink,'https://drive.google.com/delivery','dispatch must copy the planner delivery link without marking the employee submission complete');
assert.strictEqual(orderWithDelivery.submitLinks.length,0,'a Supervisor delivery link must not be mistaken for an employee submission');
...
assert.strictEqual(filter.plannerOrderExists(integrityDraft,[{id:'GR500',sourceDraftId:'different-draft'}]),false,'a duplicated visible GR number from another draft must never be treated as this draft');
