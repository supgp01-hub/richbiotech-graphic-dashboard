...
let saved;
api.mount(w.document.querySelector('main'),order,{rows:()=>[{brand:'So Pink',name:'ใหม่',hook:'HOOK A'},{brand:'So Pink',name:'ใหม่',hook:'HOOK B'},{brand:'JUDO',name:'สินค้าอื่น',hook:'ห้ามปน'}],presets:['บรีฟสำเร็จรูป'],save:async fields=>{saved=fields;}});
...
(async()=>{await button.onclick();assert.equal(saved.name,'ใหม่');assert.equal(saved.brief,'บรีฟสำเร็จรูป');assert.equal(saved.hook2,'HOOK B');assert.deepEqual(Object.keys(saved).sort(),['brief','hook','hook2','name','title']);assert.equal(order.deadline,'2026-09-12');assert.equal(order.rawLink,'preserve');
