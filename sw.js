'use strict';
const CACHE='richbiotech-shell-v344';
...
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key.startsWith('richbiotech-shell-')&&key!==CACHE).map(key=>caches.delete(key)))).then(()=>self.clients.claim()));});
self.addEventListener('fetch',event=>{
  ...
  if(!/\.(?:js|css|svg|png|jpg|jpeg|webp|woff2?)(?:\?|$)/i.test(url.pathname+url.search))return;
