(function(window,document){
  ...
  function versionCard(state,index,mode,rerender){
    ...
    if(mode==='audit'){
      ...
      var issue=document.createElement('select');issue.className='rb-av-issue-input';issue.innerHTML=optionHtml(state.issueType);issue.value=state.issueType;issue.disabled=false;issue.onchange=function(){state.issueType=this.value;state.updatedAt=Date.now();state.updatedBy=actor();};
      var note=document.createElement('textarea');note.className='rb-av-note-input';note.placeholder='ระบุว่าเวอร์ชันนี้ผิดอะไร และต้องแก้จุดใด...';note.value=state.note;note.disabled=false;note.oninput=function(){state.note=this.value;state.updatedAt=Date.now();state.updatedBy=actor();};fields.append(result,issue,note);
