(function(window,document){
  ...
  function teamCorrection(state,index,mode,rerender){
    ...
    if(mode==='employee'&&state.result==='issue'){
      ...
      var ready=hasRequiredFixImage(state),requirement=document.createElement('div');requirement.className='rb-av-fix-requirement '+(ready?'is-ready':'is-missing');requirement.innerHTML='<i>'+(ready?'✓':'!')+'</i><span><b>'+(ready?'แนบรูปงานแก้ไขแล้ว':'ต้องแนบรูปงานแก้ไข')+'</b><small>'+(ready?'พร้อมยืนยันส่งให้ Audit ตรวจ':'อย่างน้อย 1 รูปสำหรับ VER นี้ จึงจะยืนยันได้')+'</small></span>';
      var upload=document.createElement('label');upload.className='rb-av-upload is-fix'+(ready?'':' is-required');upload.tabIndex=0;upload.innerHTML='<b>↑ ลากรูปมาวาง หรือกดเลือกไฟล์</b><small>จำเป็น · อัปได้หลายรูป · JPG / PNG / WEBP</small><input type="file" accept="image/*" multiple hidden>';upload.querySelector('input').onchange=function(){addFiles(this.files,state.fixImages,function(){state.employeeSubmittedAt=0;state.correctionDraftUpdatedAt=Date.now();rerender();});};
      ...
      var note=document.createElement('textarea');note.className='rb-av-fix-note';note.placeholder='หมายเหตุถึง Audit (ไม่บังคับ)';note.value=state.fixNote||'';note.oninput=function(){state.fixNote=this.value;state.employeeSubmittedAt=0;state.correctionDraftUpdatedAt=Date.now();};stage.append(requirement,upload,link,note);
  ...
  function hasRequiredFixImage(state){
    var requested=Number(state&&state.correctionRequestedAt||0),images=state&&Array.isArray(state.fixImages)?state.fixImages:[];
    return images.some(function(image){return !!(image&&image.data)&&(!requested||Number(image.at||0)>requested);});
  }
  function closeEvidenceAlert(alert){if(alert&&alert.parentNode)alert.parentNode.removeChild(alert);}
  function showMissingEvidence(section,states){
    var old=document.getElementById('rb-av-revision-alert');if(old)old.remove();
    var versions=states.map(function(state){return'VER '+state.version;});
    states.forEach(function(state){var card=section.querySelector('[data-version-index="'+(state.version-1)+'"]');if(card)card.classList.add('rb-av-missing-evidence');});
    var alert=document.createElement('div');alert.id='rb-av-revision-alert';alert.className='rb-av-alert-overlay';alert.innerHTML='<section class="rb-av-alert-dialog" role="alertdialog" aria-modal="true" aria-labelledby="rb-av-alert-title"><button type="button" class="rb-av-alert-close" aria-label="ปิด">×</button><i class="rb-av-alert-icon">!</i><h3 id="rb-av-alert-title">ยังส่งงานแก้ไขไม่ได้</h3><p>กรุณาแนบรูปงานที่แก้ไขแล้วอย่างน้อย 1 รูปใน <b>'+versions.join(', ')+'</b></p><small>ลิงก์หรือหมายเหตุอย่างเดียวไม่ถือว่าแนบงานแก้ไขครบ</small><button type="button" class="rb-av-alert-go">ไปแนบรูปงานแก้ไข</button></section>';
    function close(){closeEvidenceAlert(alert);}alert.querySelector('.rb-av-alert-close').onclick=close;alert.onclick=function(event){if(event.target===alert)close();};alert.querySelector('.rb-av-alert-go').onclick=function(){var first=states[0],target=section.querySelector('[data-version-index="'+(first.version-1)+'"] .rb-av-upload.is-fix');close();if(target){target.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(function(){target.focus();},350);}};document.body.appendChild(alert);alert.querySelector('.rb-av-alert-go').focus();
  }
  function saveEmployee(section,rerender){
    ...
    if(order.status!=='revision'){setMessage(section,'ส่งแก้ไขรอบนี้แล้ว กำลังรอ Audit ตรวจ ไม่ต้องส่งซ้ำ');return;}
  ...
  function buildSection(panel,id,states,mode,surface,rerender){
    ...
    if(surface==='team'&&mode==='employee'&&issueCount&&currentOrder().status==='revision'){var missing=states.filter(function(state){return state.result==='issue'&&!hasRequiredFixImage(state);});var save=document.createElement('button');save.type='button';save.className='rb-av-employee-save'+(missing.length?' is-blocked':'');save.setAttribute('aria-disabled',missing.length?'true':'false');save.textContent=missing.length?'แนบรูปงานแก้ไขให้ครบก่อนยืนยัน':'✓ ยืนยันส่งงานแก้ไขให้ Audit ตรวจ';save.onclick=function(){saveEmployee(section,rerender);};section.appendChild(save);}
