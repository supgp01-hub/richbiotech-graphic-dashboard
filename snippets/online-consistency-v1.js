...
async function backups(){
 ...
 try{var data=await new Promise(function(resolve,reject){(w.rbPersistence&&w.rbPersistence.readOnline||w.fbGet)('/workflow_snapshots/order_conflict_backups_v1',function(e,d){if(e)reject(e);else resolve(d);});});var records=Object.keys(data||{}).flatMap(function(uid){return Object.values(data[uid]||{}).map(function(record){return Object.assign({},record,{_ownerUid:uid});});});body.textContent=records.length?'สำเนาข้อมูลที่ยังไม่ได้รวมเข้ากับงานหลัก • ข้อมูลออนไลน์ที่แสดงคือขณะสำรอง กรุณาตรวจงานล่าสุดก่อนแก้ไข':'ยังไม่มีข้อมูลสำรองจากอุปกรณ์ • เครื่องที่มีคิวต้องเปิดเว็บรุ่นใหม่และเชื่อมออนไลน์ก่อน';records.forEach(function(record){var op=record.operation||{},row=document.createElement('section'),heading=document.createElement('h3');heading.textContent=(record.online&&record.online.id||op.data&&op.data.id||op.path)+' · '+(record.owner||'')+' · '+new Date(record.backedUpAt||0).toLocaleString('th-TH');row.appendChild(heading);if(w.rbOrderConflictRecovery){var recover=document.createElement('button');recover.textContent='ตรวจและกู้ข้อมูลส่งงาน';recover.onclick=function(){w.rbOrderConflictRecovery.show(record);};row.appendChild(recover);}Object.keys(op.data||{}).filter(function(k){return !clientField(k)&&serverFields.indexOf(k)<0;}).forEach(function(k){var p=document.createElement('p');p.style.cssText='white-space:pre-wrap;overflow-wrap:anywhere';p.textContent=k+'\nแบบร่าง: '+JSON.stringify(op.data[k]).slice(0,10000)+'\nออนไลน์ขณะสำรอง: '+JSON.stringify(record.online&&record.online[k]||null).slice(0,10000);row.appendChild(p);});body.appendChild(row);});}catch(e){body.textContent='อ่านสำเนาไม่สำเร็จ: '+e.message;}
}
