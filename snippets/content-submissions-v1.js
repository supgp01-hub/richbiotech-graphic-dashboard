(function (w) {
  ...
  async function open(id,ownerUid){
    ...
    try{
      ...
      button.onclick=async function(){
        ...
        try{own=await saveOwn(id,submittedText,own);if(session()!==sig)return;if(textarea.value===submittedText){delete bg.dataset.dirty;if(w.rbReleaseUpdate)w.rbReleaseUpdate.confirmSaved(textarea,submittedText);status.textContent='บันทึกออนไลน์แล้ว · ประวัติและข้อความล่าสุดอัปเดตแล้ว';}else{bg.dataset.dirty='1';status.textContent='บันทึกข้อความก่อนหน้าแล้ว · ข้อความที่เพิ่งแก้ยังรอบันทึก';}}
  ...
  w.ctSubmissions={pendingCount:function(){return activeSaves;},history:history,updated:updated,readOwn:readOwn,saveOwn:saveOwn,refresh:function(){return load(true);},cell:cell,dates:dates,mount:mount,entries:entries,key:key,version:function(){session();return identity+':'+revision;}};
