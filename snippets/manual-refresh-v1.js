...
function parsePages(csv){return window.rbFacebookPagesSource.parse(csv);}
function pageKey(row){return row._pageSourceKey||window.rbFacebookPagesSource.base(row);}
function mergePages(existing,incoming){var rows=window.rbFacebookPagesSource.merge(existing,incoming);return{rows:rows,added:Math.max(0,rows.length-(existing||[]).length),updated:(incoming||[]).length};}
...
function initFbListManual(){
 window._fblDone=true;
 var cached=window.rbFacebookPagesSource.normalize(load(FB_CACHE,[])),time=Number(localStorage.getItem(FB_REFRESHED)||0);
 window._lfbData=cached;renderPages(cached);if(window._fpSyncFromCloud)window._fpSyncFromCloud(function(){renderPages(window._lfbData||cached);});
 cloudGet(FB_CLOUD).then(function(snapshot){
   var cloudTime=Number(snapshot&&snapshot.updatedAt||0),current=window._lfbData||cached;
   var merged=cloudTime>time?mergePages(current,snapshotRows(snapshot)).rows:mergePages(snapshotRows(snapshot),current).rows;
   save(FB_CACHE,merged);window._lfbData=merged;renderPages(merged);
   if(cloudTime>time)try{localStorage.setItem(FB_REFRESHED,String(cloudTime));}catch(e){}if(snapshot&&snapshot.schemaVersion===438)save('rb_fbpages_schema_v1',438);
   refreshStatus('lfb-ts',FB_REFRESHED);
 });refreshStatus('lfb-ts',FB_REFRESHED);applyPermissions();
}
function fetchFbPagesManual(){var button=document.getElementById('lfb-upd'),ts=document.getElementById('lfb-ts');if(!canRefresh()){if(ts)ts.textContent='ดูข้อมูลล่าสุดได้ แต่ไม่มีสิทธิ์อัปเดต';return Promise.resolve(false)}if(button){button.disabled=true;button.textContent='กำลังโหลด...'}return Promise.all([fetch(FB_URL).then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.text()}),cloudGet(FB_CLOUD),new Promise(function(resolve){if(window._fpSyncFromCloud)window._fpSyncFromCloud(resolve);else resolve();})]).then(function(res){var stored=mergePages(load(FB_CACHE,[]),snapshotRows(res[1])).rows,merged=mergePages(stored,parsePages(res[0])),now=Date.now();save(FB_CACHE,merged.rows);window._lfbData=merged.rows;save('rb_fbpages_schema_v1',438);stamp(FB_REFRESHED);renderPages(merged.rows);return cloudSet(FB_CLOUD,{items:merged.rows,updatedAt:now,updatedBy:window._rbUser?window._rbUser.name:'',schemaVersion:438}).then(function(ok){if(ok===false){if(ts)ts.textContent='อัปเดตในเครื่องแล้ว · บันทึกส่วนกลางไม่สำเร็จ';return false}if(ts)ts.textContent='อัปเดตแล้ว · เพิ่ม '+merged.added+' · ปรับปรุง '+merged.updated+' · รวม '+merged.rows.length;setTimeout(function(){refreshStatus('lfb-ts',FB_REFRESHED)},3500);return true})}).catch(function(){var cached=load(FB_CACHE,[]);if(cached.length)renderPages(cached);if(ts)ts.textContent='เชื่อมต่อไม่ได้ · ใช้ข้อมูลล่าสุดที่บันทึกไว้';return false}).finally(function(){if(button){button.disabled=false;button.textContent='อัปเดตข้อมูลล่าสุด'}})}
