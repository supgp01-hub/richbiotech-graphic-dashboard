...
function applyPermissions(){var allowed=canRefresh();document.querySelectorAll('#lfb-upd,#lfb2-upd,[data-rb-heavy-refresh]').forEach(function(button){button.hidden=!allowed;button.style.setProperty('display',allowed?'inline-flex':'none','important');button.setAttribute('aria-hidden',allowed?'false':'true')});refreshStatus('lfb-ts',FB_REFRESHED);refreshStatus('lfb2-ts','rb_listfacebook_refreshed_v1');refreshStatus('rb-commission-ts',COMM_REFRESHED)}
