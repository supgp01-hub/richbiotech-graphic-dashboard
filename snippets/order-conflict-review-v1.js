...
function recoverMissingAudit(op,remote){
 if(!op||!op.conflict||!['PATCH','PUT'].includes(op.method)||!remote||remote._deleted||!op.data||op.data._deleted||!w.rbSafeOrderWrite)return null;
 ...
 var next=Object.assign({},op,{token:op.token+'_audit446',method:'PATCH',data:data,baseValues:base,baseUpdatedAt:Number(remote.updatedAt||0),conflict:false,attempts:0,nextAttemptAt:0});delete next.conflictMessage;
...
function report(queue,data){
 ...
 reportBusy=true;w.rbFirebaseAuth.fetch('https://richbiotech-c4e41-default-rtdb.firebaseio.com/workflow_snapshots/order_sync_diagnostics_v1/'+encodeURIComponent(u.uid)+'/'+encodeURIComponent(device)+'.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({build:(document.querySelector&&document.querySelector('meta[name="rb-build"]')||{}).content||'unknown',employee:u.name||'',count:conflicts.length,conflicts:conflicts,checkedAt:{'.sv':'timestamp'}})}).then(function(r){if(r.ok){reportSignature=signature;reportedAt=Date.now();}}).catch(function(){}).finally(function(){reportBusy=false;});
}
