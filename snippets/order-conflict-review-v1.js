...
function report(queue,data){
 ...
 reportBusy=true;w.rbFirebaseAuth.fetch('https://richbiotech-c4e41-default-rtdb.firebaseio.com/workflow_snapshots/order_sync_diagnostics_v1/'+encodeURIComponent(u.uid)+'/'+encodeURIComponent(device)+'.json',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({build:'fix446',employee:u.name||'',count:conflicts.length,conflicts:conflicts,checkedAt:{'.sv':'timestamp'}})}).then(function(r){if(r.ok)reportSignature=signature;}).catch(function(){}).finally(function(){reportBusy=false;});
}
