(function(w){
'use strict';
function allowed(){var u=w._rbUser||{};return u.role==='sup'&&String(u.name||'').trim().toUpperCase()==='VIEW';}
...
function mount(){var u=w._rbUser||{};document.documentElement.dataset.contentHistoryAccess=JSON.stringify({supervisor:u.role==='sup',view:String(u.name||'').trim().toUpperCase()==='VIEW',ready:!!u.uid});var actions=document.querySelector('.ct-actions'),head=document.querySelector('.ct-table thead tr');if(!actions||!head)return;var th=head.querySelector('.cth-heading');if(allowed()&&!th){th=document.createElement('th');th.className='cth-heading';th.textContent='Deadline ครั้งล่าสุด';var hook=Array.from(head.children).find(function(n){return n.textContent.trim().toLowerCase()==='hook';});if(hook)hook.insertAdjacentElement('afterend',th);}if(!allowed()&&th)th.remove();var b=document.getElementById('cth-open');if(!b){b=document.createElement('button');b.id='cth-open';b.className='ct-btn ct-btn-secondary';b.textContent='◷ ประวัติการสั่ง HOOK';b.onclick=function(){open();};actions.appendChild(b);}b.hidden=!allowed();if(!allowed()){close();document.querySelectorAll('.cth-warning').forEach(function(n){n.remove();});}}
