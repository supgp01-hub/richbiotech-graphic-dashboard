...
function normalizeStage(stage){return stage==='waiting'?'working':stage;}
function mergeFollowupMaps(localMap,remoteMap){var merged={},local=localMap&&typeof localMap==='object'?localMap:{},remote=remoteMap&&typeof remoteMap==='object'?remoteMap:{};Object.keys(remote).forEach(function(key){if(remote[key]&&typeof remote[key]==='object')merged[key]=remote[key];});Object.keys(local).forEach(function(key){var localEntry=local[key];if(!localEntry||typeof localEntry!=='object')return;var remoteEntry=merged[key],localTime=Number(localEntry.updatedAt)||0,remoteTime=Number(remoteEntry&&remoteEntry.updatedAt)||0;if(!remoteEntry||localTime>=remoteTime)merged[key]=localEntry;});return merged;}
function rowMeta(row){
  var requires=needsSystemFollowup(row),sourceSaved=followups[row._key]||{},saved=Object.assign({},sourceSaved),normalized=normalizeStage(saved.stage);
  if(normalized!==saved.stage)saved.stage=normalized;
  if(!requires){saved.stage='none';saved.nextDate='';}
  var marked=requires&&isMarked(row),recommended=requires&&!marked,stage=requires&&saved.stage&&saved.stage!=='none'?saved.stage:requires?(marked?'new':recommended?'suggested':'none'):'none';
  return{saved:saved,sourceSaved:sourceSaved,requires:requires,marked:marked,recommended:recommended,stage:stage};
}
function stageLabel(stage){return{new:'ยังไม่เริ่ม',working:'กำลังแก้',done:'แก้แล้ว',suggested:'ระบบแนะนำ'}[normalizeStage(stage)]||'ไม่ติดตาม';}
function stageClass(stage){return{new:'new',working:'working',done:'done',suggested:'suggested'}[normalizeStage(stage)]||'none';}
...
function stageOptions(current){
  current=normalizeStage(current);
  return[['new','ยังไม่เริ่ม'],['working','กำลังแก้'],['done','แก้แล้ว']].map(function(pair){return'<option value="'+pair[0]+'"'+(pair[0]===current?' selected':'')+'>'+pair[1]+'</option>';}).join('');
}
...
function accountDropdownValues(key,current,data){
  var values=[],seen={};
  function add(value){
    value=String(value==null?'':value).trim();
    var normalized=value.toLocaleUpperCase();
    if(!value||seen[normalized])return;
    seen[normalized]=true;values.push(value);
  }
  add(current);
...
function populateAccountDropdown(key,current,data,emptyLabel){
  var select=document.getElementById('lfbi-'+key);if(!select)return;
  select.innerHTML='<option value="">'+esc(emptyLabel)+'</option>'+accountDropdownValues(key,current,data).map(function(value){return'<option value="'+esc(value)+'">'+esc(value)+'</option>';}).join('');
...
function updateAccountFollowupDate(value){
...
function followupTiming(saved,now){
  ...
  if(saved.stage==='none')return{level:0,className:'',daysLeft:null,nextDate:'',label:'ไม่ติดตาม'};
...
function renderStages(counts){
  ...
  var stages=[['new','ต้องเริ่มแก้','รายการที่ทีมมาร์คไว้'],['working','กำลังแก้','มีผู้รับผิดชอบแล้ว'],['done','แก้แล้ว','เก็บประวัติไว้']];
...
function renderAccountDetail(){
  ...
  populateAccountDropdown('emp',row.emp,window._listfbData||[],'— ไม่ระบุพนักงาน —');
  populateAccountDropdown('prod',row.prod,window._listfbData||[],'— ไม่ระบุสินค้า —');
...
function updateCredentialsDrawer(){
  var drawer=document.getElementById('lfb-credentials-drawer');if(!drawer)return;
  drawer.classList.toggle('is-open',credentialsOpen);drawer.setAttribute('aria-hidden',credentialsOpen?'false':'true');
  var open=document.getElementById('lfb-credentials-open');if(open)open.setAttribute('aria-expanded',credentialsOpen?'true':'false');
  var read=drawer.querySelector('.lfb-credentials-read'),edit=drawer.querySelector('.lfb-credentials-edit');
  if(read)read.hidden=credentialsEditing;if(edit)edit.hidden=!credentialsEditing;
  ['passFb','email','emailPass','twofa'].forEach(function(key){var input=document.getElementById('lfbi-'+key);if(input)input.disabled=!credentialsEditing;});
}
function openCredentialsDrawer(editing){credentialsOpen=true;credentialsEditing=!!editing;updateCredentialsDrawer();}
function closeCredentialsDrawer(){credentialsOpen=false;updateCredentialsDrawer();}
function cancelCredentialsEdit(){
  var detail=document.getElementById('lfb-account-detail'),row=rowByKey(detail&&detail.getAttribute('data-account-key')||selectedKey)||{};
  ['passFb','email','emailPass','twofa'].forEach(function(key){var input=document.getElementById('lfbi-'+key);if(input)input.value=row[key]||'';});
  credentialsEditing=false;updateCredentialsDrawer();
}
...
function persistAccountDetail(reopenCredentials){
  var editor=document.getElementById('lfb-account-detail'),saveKey=editor&&editor.getAttribute('data-account-key')||selectedKey;
...
function deleteAccount(){
  var detail=document.getElementById('lfb-account-detail'),accountKey=detail&&detail.getAttribute('data-account-key')||selectedKey;
  var row=rowByKey(accountKey),button=document.getElementById('lfb-account-delete'),state=document.getElementById('lfb-account-save-state');
  if(!row||typeof window._lfbDeleteAccountRecord!=='function'){if(state)state.textContent='ไม่พบบัญชีที่เลือก';return;}
  if(!window.confirm('ลบบัญชี “'+String(row.name||'-')+'” ออกจากระบบ?\n\nบัญชีนี้จะถูกลบออกจากทุก Chrome และจะไม่กลับมาเมื่ออัปเดตข้อมูล การดำเนินการนี้ย้อนกลับไม่ได้'))return;
...
function bindHybrid(root){
  ...
  root.addEventListener('click',function(event){
    ...
    if(event.target.closest('#lfb-account-delete')){deleteAccount();return;}
